...
def get_xgb_trees_table_entries(model_file, keys, feat_dict, key_encode_bits, pkts=None):
    ...
    for line in lines:
        ...
        if "yes" in line:
            ...
            nodes[m.group(4)][keys.index(feat) * 2] = min(
                nodes[m.group(4)][keys.index(feat) * 2], feat_dict[feat].index(thre) + 1
            )
            nodes[m.group(5)] = nodes[m.group(1)].copy()
            nodes[m.group(5)][keys.index(feat) * 2 + 1] = min(
                nodes[m.group(5)][keys.index(feat) * 2 + 1], -feat_dict[feat].index(thre) - 1
            )
